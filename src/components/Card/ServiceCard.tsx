...
const ServiceCard: FC<ServiceCardProps> = ({ title, content, icon, numeric, id, horizontal }) => {
  if (horizontal) {
    return (
      <div className="py-6 w-full md:w-max flex  md:items-start  md:gap-6 gap-3">
        <div>
          {icon ? <div className="md:w-16 md:h-16 h-11 w-11 rounded-full bg-[#F9F9F9] flex items-center justify-center">{icon}</div> : <span className="text-4xl md:text-6xl text-primaryColor font-bold">{String(id).padStart(2, "0")}</span>}
        </div>
